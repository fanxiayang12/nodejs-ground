 ...
 * zhanxp@me.com
 */
function MysqlDB(database) {
    this.pool = null;
    this.debug = config.debug;
    this.database = database;
    this.connect = function(connStrJson) {
        var connStrJson = config.mysql[this.database];
        if (this.debug) {
    ...
    this.loadMoreTable = async function(table, conditions) {
        conditions = conditions || {};
        conditions.where = conditions.where || "1=1";
        conditions.cols = conditions.cols || '*';
        var sql = `select ${conditions.cols} from ${table} where ${conditions.where}`;
        if (conditions.limit)
            sql = sql + ` limit ${conditions.limit}`;
        var rows = await this.query(sql, conditions.params);
        return rows;
    }
